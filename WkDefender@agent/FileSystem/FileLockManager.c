...
/* 关键进程名单 (L91-102 CRITICAL_PROC_NAMES) */
...
typedef struct _WKD_FLM_GLOBAL {
    ...
    CRITICAL_SECTION CallbackLock;   /* 回调注册/读取 (m_callbackMutex) */
} WKD_FLM_GLOBAL;
...
/*++
 * FlmNormalizePath — GetFullPathNameW 两遍归一化 (L719-734)。
...
/*++
 * FlmEnableDebugPrivilege — 启用 SeDebugPrivilege (L736-747)。
...
/*++
 * FlmIsCriticalProcessName — 关键进程名判定 (L97-102)。
...
/*++
 * FlmResolveObjectTypeIndex — 探测 File 对象 ObjectTypeIndex (L761-812)。
...
/*++
 * FlmClassifyLockType — 访问掩码 → 锁类型 (L814-824)。
 *--*/
 ...
 *   VerifyFileSignature L956-968)。签名成功返回 TRUE。
 *--*/
static BOOLEAN
...
/*++
 * FlmDetectProcessInjection — 轻量注入探测 (L970-996)。
...
/*++
 * FlmEnrichProcessInfo — 富化锁 owner (L924-938)。
    ...
    )
{
    ...
    /* 信任分析 (AnalyzeProcessTrust L947-955) */
...
/*++
 * FlmRmViaSession — Restart Manager 会话辅助 (L826-853/L570-588)。
...
/*++
 * FlmGetLockingProcessesRM — RM 来源锁 owner 枚举 (L826-853)。
 *   填充 Owners 数组, 返回填充数。
 *--*/
static ULONG
...
/*++
 * FlmGetLockingProcessesHandleEnum — 句柄枚举来源锁 owner (L854-922)。
    ...
    )
{
...

    {
        ...
        for (idx = 0; idx < sysInfo->NumberOfHandles && filled < MaxOwners; idx++) {
            ...
            /* foundPids 去重: 已发现的 PID 仅保留写/删权限句柄 (L884) */
            ...
            /* DuplicateHandle + 文件类型 + 文件 ID 匹配 (L894-905) */
...
/*++
 * FlmCollectLockingProcesses — RM + 句柄枚举双路合并去重 (L259-290)。
...
/*++
 * FlmDetectLockPatternInternal — 锁行为模式判定 (L1031-1052)。
    ...
    )
{
    ...
    for (i = 0; i < Info->LockCount; i++) {
        ...
        if (!o->IsSigned && o->IsSuspicious && o->LockType == WkdLock_Exclusive) {
            ...
            dot = wcsrchr(name, L'.');       /* 取文件名段扩展名 (fs::path::extension) */
            if (dot) {
                BOOLEAN isDoc = FALSE;
...
/*++
 * FlmAnalyzeThreatInternal — 锁上下文 APT 威胁评估 (L998-1029)。
...
/* FlmReportProgress — 解锁进度回调 (ReportProgress L1099-1103)。
...
/*++
 * FlmCheckKernelDriver — 探测 wkd 驱动 FLT 端口可用性 (CheckKernelDriver
 ...
 *   ConnectToKernelDriverInternalLocked L694-709。
 ...
 *   KernelUnlockFileOp L648-684。协议结构/端口名对齐 SS,
...
/* EnumerateHandles — 仅句柄枚举级深层枚举 (EnumerateHandles L1108-1110,
...
/*++
 * FileLockManager_UnlockFile — 五级升级链 (L365-437)。
...
/*++
 * FileLockManager_CloseHandle — 复制句柄关闭 (CloseHandleOp L492-528)。
...
/*++
 * FileLockManager_TerminateProcess — 终止持锁进程 (TerminateProcessOp
    ...
    )
{
    ...
    /* L532-538: critical/system 保护 + 终止前回调确认 (Force 时跳过) */
    if (!Force) {
