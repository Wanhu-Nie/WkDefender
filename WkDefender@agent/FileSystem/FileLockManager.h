...
#define WKD_FLM_MAX_PROC_PATH      260     /* 进程路径缓冲 (内核协议 filePath[260]) */
...
/* 运行时统计 (FileLockManagerStatistics, 9 计数器) */
...
/* GetLockType — 读/写双 open 探测分类 (GetLockType)。 */
WKD_LOCK_TYPE
FileLockManager_GetLockType(
    _In_ PCWSTR FilePath
    );

/* CanDeleteFile — DELETE 访问探测 (CanDeleteFile)。 */
BOOLEAN
...
/* UnlockFileWithMethod — 指定方法解锁 (UnlockFile(method))。 */
_Check_return_
NTSTATUS
...
/* ForceUnlockFile — 全部方法强制解锁 (ForceUnlockFile)。 */
_Check_return_
NTSTATUS
 ...
 * PID 复用复核 + 进程退出复核, CloseHandleOp)。 */
BOOLEAN
FileLockManager_CloseHandle(
    _In_ const PWKD_LOCK_OWNER Owner
    );

/* TerminateProcess — 终止持锁进程 (TerminateProcessOp)。
 ...
 * 评分公式AnalyzeThreatInternal (未签名+15/注入+40/系统独占+30/
