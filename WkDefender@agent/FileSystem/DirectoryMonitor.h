...
/*  (搁置项)。                                      */
/*                                                  */
/*  2026-09 线程池化 (CreateThreadpoolWait) + 递归   */
/*  全树监控已启用 (bWatchSubtree=TRUE)。已知遗留:   */
/*  ① 64KB 缓冲递归下易溢出 (ErrorCb 上报, 无补扫); */
/*  ② 回调链同步扫描耗时 (消费端异步化待落地);      */
/*  ③ 浅层路径被子树重复覆盖, 容忍不去重。          */
/**************************************************/
...
#define WKD_DIR_NOTIFY_BUFFER       65536   /* RDCW 通知缓冲: MS 远程共享硬上限 (SS kNotifyBufferBytes); 递归全树下易溢出(遗留见头注释) */
...
/* 监控配置 (DirectoryMonitorConfig, 裁剪 autoDiscover/媒体/网络项) */
...
typedef struct _WKD_DIR_MONITORED_PATH {
    ...
    LARGE_INTEGER   CreatedTime;        /* 监控创建时间 (createdTime) */
} WKD_DIR_MONITORED_PATH, *PWKD_DIR_MONITORED_PATH;

/* 运行统计 (DirectoryMonitorStatistics, C 原子) */
...
/*++
Routine Description:
    按 MonitorId 查询监控路径快照 (GetMonitorById)。
...
/*++
Routine Description:
    热更新配置 (SetConfiguration)。worker 每批快照读取,
    即时生效; 非法配置拒绝。
...
/*++
Routine Description:
    平均事件处理时间 (ms, GetAverageProcessingTimeMs)。
...
/*++
Routine Description:
    枚举转字符串 (GetPathCategoryName / GetFileSystemActionName /
...
/*++
Routine Description:
    ToJson 序列化 (各 ToJson, 死代码工具, 无消费方)。
    Out 始终 NUL 结尾, 超长截断。
