...
#define WKD_MPM_VERSION_MAJOR       3       /* MountPointMonitorConstants::VERSION_MAJOR */
#define WKD_MPM_VERSION_MINOR       0
#define WKD_MPM_VERSION_PATCH       0

#define WKD_MPM_MAX_DEVICE_HISTORY  10000   /* 设备历史 cap (MAX_DEVICE_HISTORY) */
#define WKD_MPM_EVENT_QUEUE_CAPACITY 1000   /* ※死代码: SS 头文件定义未使用 (.cpp 无事件队列) */
#define WKD_MPM_POLLING_INTERVAL_MS 1000    /* 轮询兜底间隔 (POLLING_INTERVAL_MS) */

#define WKD_MPM_RAPID_CYCLE_THRESHOLD  3    /* 快速挂载循环阈值: 30s 窗内 3 次 (RAPID_CYCLE_THRESHOLD) */
#define WKD_MPM_RAPID_CYCLE_WINDOW_SEC 30   /* RAPID_CYCLE_WINDOW_SEC */

#define WKD_MPM_MAX_DRIVE_LETTERS   26      /* A-Z 盘符固定映射数组 */
#define WKD_MPM_SERIAL_MAX          128     /* 设备序列号定长 (SetupAPI instance ID 尾部, 无上限→防御性) */
#define WKD_MPM_VID_MAX             16      /* VID/PID 4 hex + NUL */
#define WKD_MPM_FRIENDLY_NAME_MAX   256     /* 友好名 (friendlyName[256]) */
...
/* 监控状态机 (MountPointMonitorStatus, 全量) */
...
/* 挂载事件类型 (MountEvent, 全量; Media / Network / Virtual 为 SS 预留未触发) */
...
/* 已挂载驱动器信息 (DriveInfo, std::wstring → 定长 WCHAR 数组) */
...
/* 监控配置 (MountPointMonitorConfig, 无工厂字段, 工厂独立函数) */
...
/* 运行统计 (MountPointMonitorStatistics, C 原子) */
...
/* 恶意设备表条目 (数据驱动, DetectThreats 硬编码两条, 仿 g_IocLolbinDb 可扩展) */
...
/*
 * 配置工厂 (MountPointMonitorConfig::CreateDefault/CreateHighSecurity)
 */
VOID
WkdMpmConfigDefault(
...
/*
 * 生命周期 (Initialize/Shutdown/Start/Stop/Is*)
 */
...
/*
 * 驱动器枚举 (GetMountedDrives 等; GetRemovableDrives/GetNetworkDrives 死代码无消费者)
 */
ULONG
...
/*
 * 回调 (SetMountEventCallback/SetPolicyCallback/UnregisterCallbacks)
 */
VOID
