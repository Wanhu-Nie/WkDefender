...
typedef struct _WKD_MPM_STATE {
    ...
    /* 锁组 (m_mutex/m_drivesMutex/m_historyMutex/
    ...
    /* 当前盘表 (盘符 A-Z 固定映射, m_mountedDrives) */
...
/*
 * WkdMpmConfigDefault
 *   默认配置 (MountPointMonitorConfig::CreateDefault):
 ...
 *   高安全配置 (MountPointMonitorConfig::CreateHighSecurity):
 ...
 *   Config 为空 → 默认配置。MountPointMonitor::Initialize。
 */
NTSTATUS
WkdMpm_Initialize(
    ...
    )
{
    ...
    /* 初始枚举 (Initialize: GetLogicalDrives 填充 m_mountedDrives) */
    ...
    )
{
    ...
    /* 防御性 fallback (StopMonitoring): 线程异常退出时消息窗口仍存活
 ...
 *   Impl::DetectRapidMountCycle (steady_clock → GetTickCount64)。
 */
static BOOLEAN
WkdMpmDetectRapidMountCycle(
    _In_ WCHAR DriveLetter
...
/*
 * WkdMpmProcessDriveArrival
 *   盘符到达全流程 (Impl::ProcessDriveArrival):
...
/*
 * WkdMpmDeviceNotifyWndProc
 *   设备通知窗口过程 (Impl::DeviceNotifyWndProc):
...
/*
 * WkdMpmMonitorThreadProc
 *   监控线程主循环 (Impl::MonitorThreadProc):
 ...
 *   RefreshDriveList; 监控线程 1s 轮询兜底调用锁内版。
 */
VOID
WkdMpm_RefreshDriveList(
    VOID
    ...
    )
{
    ...
    /* 选择排序 (LastSeen 新→旧, std::sort desc) */
    for (i = 0; i + 1 < count; i++) {
        ULONG best = i;
        for (j = i + 1; j < count; j++) {
...
/*
 * WkdMpmUpdateDeviceHistory
 *   设备历史更新 (Impl::UpdateDeviceHistory):
...
/*
 * WkdMpmHistoryEntryInit
 *   历史条目初始化 (UpdateDeviceHistory 新条目字段)。
 */
static VOID
WkdMpmHistoryEntryInit(
    _Out_ PWKD_MPM_DEVICE_HISTORY_ENTRY Entry,
 ...
 *   白名单移除设备序列号 (RemoveFromWhitelist)。
 */
VOID
WkdMpm_RemoveFromWhitelist(
    _In_ PCWSTR SerialNumber
 ...
 *   注册挂载事件回调 (SetMountEventCallback)。
 */
VOID
WkdMpm_SetMountEventCallback(
 ...
 *   更新配置 (SetConfiguration)。策略枚举校验防非法值静默禁用。
 */
NTSTATUS
WkdMpm_SetConfiguration(
 ...
 *   平均事件处理耗时 (us → ms, GetAverageProcessingTimeMs)。
 */
DOUBLE
WkdMpm_GetAverageProcessingTimeMs(
    VOID
