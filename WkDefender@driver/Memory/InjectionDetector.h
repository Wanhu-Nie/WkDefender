...
#include <ntddk.h>
/* 【WkD 迁移说明】(2026-09-11)
 * 本头文件为 WkD 补建：SS InjectionDetector 迁移时同名头未随迁，
 * 导致 WkD 工程仅有 InjectionDetector.c（其 L60 #include "InjectionDetector.h"
 * 悬空），现按 SS 原头（604 行）全量迁移补建，接口与内容保持一致：
 *   - 类型 / 接口 / IRQL 约束均同 SS 原版；
 *   - _VAD_TRACKER 仅保留前向声明。VadTracker 模块整体不迁移，
 *     其职责由 WkD 既有 Memory\MemoryRegion.c（VAD 枚举、保护判断、
 *     区域追踪、风险聚合）覆盖，InjInitialize 的 VadTracker 参数
 *     WkD 调用方一律传 NULL；
 *   - include 保持 SS 原版 <ntifs.h> / <ntddk.h>（WkD 编译环境可用）。
 */
