...
// 【WkD 迁移说明】
// 源文件（SS ShellcodeDetector.c）此处原为 #include "../Utilities/MemoryUtils.h"，
// WkD 项目中不存在该工具头，且本文件不使用其任何 API，故迁移时删除。
// 内存分配/释放已按 WkD 规范替换：ShadowStrikeAllocatePoolWithTag -> ExAllocatePool2，
...
--*/
{
    ...
    *Detector = NULL;
    ...
    //
    detector = (PSD_DETECTOR)ExAllocatePool2(
        POOL_FLAG_NON_PAGED,
        sizeof(SD_DETECTOR),
        SD_POOL_TAG_CONTEXT
    );
    ...
    if (prevState != SD_STATE_UNINITIALIZED) {
        ExFreePoolWithTag(detector, SD_POOL_TAG_CONTEXT);
...
--*/
{
    ...
    *Result = NULL;
    ...
    //
    result = (PSD_DETECTION_RESULT)ExAllocatePool2(
        POOL_FLAG_PAGED,
...
--*/
{
    ...
    *Result = NULL;
    ...
    if (!NT_SUCCESS(status)) {
        ExFreePoolWithTag(buffer, SD_POOL_TAG_BUFFER);
        SdpReleaseReference(Detector);
        return status;
    }
...
--*/
{
    ...
    *ResultCount = 0;
    ...
    //
    regions = (PSD_REGION_DESC)ExAllocatePool2(
        POOL_FLAG_PAGED,
    ...
    status = PsLookupProcessByProcessId(ProcessId, &process);
    if (!NT_SUCCESS(status)) {
        ExFreePoolWithTag(regions, SD_POOL_TAG_BUFFER);
        SdpReleaseReference(Detector);
        return status;
    }
    ...
    )
{
    ...
    // Allocate entry
    //
    entry = (PSD_API_HASH_ENTRY)ExAllocatePool2(
        POOL_FLAG_NON_PAGED,
    ...
    if (Detector->ApiHashes.HashCount >= SD_MAX_API_HASHES) {
        ...
        KeLeaveCriticalRegion();
        ExFreePoolWithTag(entry, SD_POOL_TAG_PATTERN);
        SdpReleaseReference(Detector);
    ...
    if (Detector->ApiHashes.HashTable == NULL) {
        ...
        //
        Detector->ApiHashes.HashTable = ExAllocatePool2(
            POOL_FLAG_NON_PAGED,
            sizeof(LIST_ENTRY),
            SD_POOL_TAG_PATTERN
        );
...
Cleanup:
    if (FileBuffer != NULL) {
        ExFreePoolWithTag(FileBuffer, SD_POOL_TAG_BUFFER);
    }
    ...
    )
{
    if (Result != NULL) {
        ...
        ExFreePoolWithTag(Result, SD_POOL_TAG_RESULT);
    }
}
...
--*/
{
    ...
    #define SD_ADD_HASH_INTERNAL(hash, api, dll) \
        do { \
            PSD_API_HASH_ENTRY _entry = (PSD_API_HASH_ENTRY)ExAllocatePool2( \
                POOL_FLAG_NON_PAGED, sizeof(SD_API_HASH_ENTRY), SD_POOL_TAG_PATTERN); \
            if (_entry != NULL) { \
                ...
                if (Detector->ApiHashes.HashTable == NULL) { \
                    Detector->ApiHashes.HashTable = ExAllocatePool2( \
                        POOL_FLAG_NON_PAGED, sizeof(LIST_ENTRY), SD_POOL_TAG_PATTERN); \
                    if (Detector->ApiHashes.HashTable == NULL) { \
                        ExFreePoolWithTag(_entry, SD_POOL_TAG_PATTERN); \
                        return; \
                    } \
                    InitializeListHead((PLIST_ENTRY)Detector->ApiHashes.HashTable); \
                } \
    ...
    )
{
    ...
    if (hashTable != NULL) {
        for (entry = hashTable->Flink; entry != hashTable; entry = next) {
            ...
            ExFreePoolWithTag(hashEntry, SD_POOL_TAG_PATTERN);
        }

        ExFreePoolWithTag(hashTable, SD_POOL_TAG_PATTERN);
        Detector->ApiHashes.HashTable = NULL;
    }
    ...
    )
{
    ...
    if (sigTable != NULL) {
        for (entry = sigTable->Flink; entry != sigTable; entry = next) {
            ...
            RemoveEntryList(entry);
            ExFreePoolWithTag(sigEntry, SD_POOL_TAG_PATTERN);
        }

        ExFreePoolWithTag(sigTable, SD_POOL_TAG_PATTERN);
        Detector->Signatures.SignatureDatabase = NULL;
    }
