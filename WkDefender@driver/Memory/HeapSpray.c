...
#include "HeapSpray.h"
// 【WkD 迁移说明】
// 源文件（SS HeapSpray.c）此处原为：
//   #include "../Utilities/MemoryUtils.h"
//   #include "../Utilities/StringUtils.h"
// WkD 项目中不存在这两个工具头，本文件所需 API 已本地化：
//   内存分配/释放：ShadowStrikeAllocatePoolWithTag -> ExAllocatePool2、
//                  ShadowStrikeFreePoolWithTag -> ExFreePoolWithTag；
//   模式哈希：ShadowStrikeHashBytes（FNV-1a）改为本地实现，
//            见 HspCalculatePatternHash 函数。
// 故两行 include 迁移时删除。
#ifdef ALLOC_PRAGMA
...
--*/
{
    ...
    *Detector = NULL;
    ...
    //
    detectorInternal = (PHS_DETECTOR_INTERNAL)ExAllocatePool2(
        POOL_FLAG_NON_PAGED,
    ...
    detector->AllocationPool.PoolMemory = (PHS_ALLOCATION_RECORD)ExAllocatePool2(
        POOL_FLAG_NON_PAGED,
    ...
    if (detector->AllocationPool.PoolMemory == NULL) {
        ExFreePoolWithTag(detectorInternal, HS_POOL_TAG_CONTEXT);
        return STATUS_INSUFFICIENT_RESOURCES;
    }
...
--*/
{
    ...
    *Result = NULL;
    ...
    //
    result = (PHS_SPRAY_RESULT)ExAllocatePool2(
        POOL_FLAG_NON_PAGED,
...
{
    if (Result != NULL) {
        ExFreePoolWithTag(Result, HS_POOL_TAG_RESULT);
    }
}
...
Routine Description:
    ...
    【WkD 迁移说明】
    源实现委托给 SS 工具头（StringUtils.h）的 ShadowStrikeHashBytes，
    该函数为标准字节级 FNV-1a 32 位哈希
    （offset basis 0x811C9DC5、prime 0x01000193）。
    WkD 项目中无对应工具头，按 WkD 规范迁移为本地内联实现，
    算法与源实现完全一致，结果保持兼容。

--*/
{
    ULONG hash = 2166136261UL;      // FNV-1a offset basis (0x811C9DC5)
    ULONG i;
    ...
    for (i = 0; i < Size; i++) {
        hash ^= Data[i];
        hash *= 16777619UL;         // FNV-1a prime (0x01000193)
    }
