...
//【WkD 迁移说明】(2026-09-11)
//   源文件：SS PhantomSensor/Memory/ROPDetector.c。适配点如下：
//   - ShadowStrikeAllocatePoolWithTag / ShadowStrikeFreePoolWithTag
//     → ExAllocatePool2 / ExFreePoolWithTag（WDK 原生，NonPagedPoolNx → POOL_FLAG_NON_PAGED）；
//   - ShadowStrikeLookasideInit / Cleanup / Allocate / Free → Common/Lookaside.h 的
//     CoInitializeLookaside / CoDeleteLookaside / CoAllocateLookaside / WkdLookasideFree
//     （WKD_LOOKASIDE 代替 SS 的 SHADOWSTRIKE_LOOKASIDE 封装；SS 原 .Initialized
//      检测改用显式 BOOLEAN LookasideInitialized，对齐 FileSystem/PostCreateContext.c 惯例；
//      原深度参数 ROP_GADGET_LOOKASIDE_DEPTH 由 Co 封装内部固定为系统默认，常量已删除）；
//   - ShadowStrikeIsKernelAddress → Address >= MmSystemRangeStart（保留 SS 原语义；
//     WkD 其它模块另有 MM_HIGHEST_USER_ADDRESS 惯例，此处忠实迁移不做改写）；
//   - ShadowStrikeIsAligned → 内联对齐判断；
//   - ShadowStrikeSecureFree → RtlSecureZeroMemory + ExFreePoolWithTag（PASSIVE 路径全量擦除）；
//   - SS 最大分配尺寸常量 → ROP_MAX_MODULE_SIZE（本地常量，256MB）。
//   未改动：ROP/JOP 检测算法本体与公共 API 签名（ROPDetector.h 原样保留）。
...
#define ROP_MAX_MODULE_SIZE             (256 * 1024 * 1024)  // matches SS SHADOWSTRIKE_MAX_ALLOCATION_SIZE
...
typedef struct _ROP_DETECTOR_INTERNAL {
    ...
    //
    WKD_LOOKASIDE GadgetLookaside;
    BOOLEAN LookasideInitialized;   // drops SS .Initialized field; TRUE after CoInitializeLookaside
...
--*/
{
    ...
    *Detector = NULL;
    ...
    //
    internalDetector = (PROP_DETECTOR_INTERNAL)ExAllocatePool2(
        POOL_FLAG_NON_PAGED,
    ...
    status = CoInitializeLookaside(
        ...
        sizeof(ROP_GADGET),
        ROP_POOL_TAG_GADGET,
        WkdLookasideNonPaged   // SS passed depth=ROP_GADGET_LOOKASIDE_DEPTH + Non-paged; Co wrapper uses system default
        );
    ...
    internalDetector->LookasideInitialized = TRUE;
...
Cleanup:
    if (internalDetector != NULL) {
        if (internalDetector->LookasideInitialized) {
            CoDeleteLookaside(&internalDetector->GadgetLookaside);
        }

        ExFreePoolWithTag(internalDetector, ROP_POOL_TAG_CONTEXT);
    }
...
--*/
{
    ...
    while (!IsListEmpty(&Detector->ScannedModules)) {
        ...
        if (module->ModuleName.Buffer != NULL) {
            ExFreePoolWithTag(module->ModuleName.Buffer, ROP_POOL_TAG_CONTEXT);
        }
        ExFreePoolWithTag(module, ROP_POOL_TAG_CONTEXT);
    }
    ...
    while (!IsListEmpty(&internalDetector->CallbackList)) {
        ...
        ExFreePoolWithTag(callback, ROP_POOL_TAG_CONTEXT);
    }
...
--*/
{
    ...
    if (ModuleBase == NULL || ModuleSize == 0 || ModuleSize > ROP_MAX_MODULE_SIZE) {
        return STATUS_INVALID_PARAMETER;
    }
    ...
    //
    moduleEntry = (PROP_SCANNED_MODULE)ExAllocatePool2(
        POOL_FLAG_NON_PAGED,
    ...
    if (ModuleName != NULL && ModuleName->Length > 0 && ModuleName->Buffer != NULL) {
        ...
        moduleEntry->ModuleName.Buffer = (PWCH)ExAllocatePool2(
            POOL_FLAG_NON_PAGED,
...
--*/
{
    ...
    *Result = NULL;
    ...
    //
    context = (PROP_ANALYSIS_CONTEXT)ExAllocatePool2(
        POOL_FLAG_NON_PAGED,
    ...
    if (!NT_SUCCESS(status)) {
        ExFreePoolWithTag(context, 'xCOR');
        RoppReleaseRundown(Detector);
        return status;
    }
...
--*/
{
    ...
    *Result = NULL;
    ...
    if ((((ULONG_PTR)StackBuffer) & (ROP_STACK_ALIGNMENT - 1)) != 0) {   // WkD: inline ShadowStrikeIsAligned
...
Routine Description:
    ...
    ExFreePoolWithTag consistently.
...
--*/
{
    ...
    //
    ExFreePoolWithTag(Result, ROP_POOL_TAG_CONTEXT);
}
...
--*/
{
    ...
    callbackEntry = (PROP_CALLBACK_ENTRY)ExAllocatePool2(
        POOL_FLAG_NON_PAGED,
    ...
    if (internalDetector->CallbackCount >= ROP_MAX_CALLBACKS) {
        ExReleasePushLockExclusive(&internalDetector->CallbackLock);
        KeLeaveCriticalRegion();
        ExFreePoolWithTag(callbackEntry, ROP_POOL_TAG_CONTEXT);
        RoppReleaseRundown(Detector);
        return STATUS_QUOTA_EXCEEDED;
    }
...
--*/
{
    ...
    if (foundEntry != NULL) {
        ...
        ExFreePoolWithTag(foundEntry, ROP_POOL_TAG_CONTEXT);
    }
}
    ...
    )
{
    ...
    gadget = (PROP_GADGET)CoAllocateLookaside(&Detector->GadgetLookaside);
...
{
    if (Gadget != NULL) {
        WkdLookasideFree(&Detector->GadgetLookaside, Gadget);
    }
}
...
// so that RopFreeResult can safely use ExFreePoolWithTag.
//
static
NTSTATUS
RoppAllocateChainEntry(
    ...
    )
{
    ...
    entry = (PROP_CHAIN_ENTRY)ExAllocatePool2(
        POOL_FLAG_NON_PAGED,
...
{
    if (Entry != NULL) {
        ExFreePoolWithTag(Entry, ROP_POOL_TAG_CHAIN);
    }
}
    ...
    )
{
    ...
    result = (PROP_DETECTION_RESULT)ExAllocatePool2(
        POOL_FLAG_NON_PAGED,
    ...
    )
{
    ...
    if (Context->StackBuffer != NULL) {
        RtlSecureZeroMemory(Context->StackBuffer, Context->StackBufferSize);
        ExFreePoolWithTag(Context->StackBuffer, ROP_POOL_TAG_CONTEXT);   // WkD: inline ShadowStrikeSecureFree (full wipe at PASSIVE)
...
--*/
{
    ...
    Context->StackBuffer = (PULONG_PTR)ExAllocatePool2(
        POOL_FLAG_NON_PAGED,
    ...
    if (!NT_SUCCESS(status)) {
        ExFreePoolWithTag(Context->StackBuffer, ROP_POOL_TAG_CONTEXT);
        Context->StackBuffer = NULL;
        Context->StackBufferSize = 0;
        return status;
    }
...
--*/
{
    ...
    //
    if (Address >= MmSystemRangeStart) {   // WkD: ShadowStrikeIsKernelAddress -> MmSystemRangeStart (SS semantic)
        return TRUE;
    }
