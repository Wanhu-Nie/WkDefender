...
//
// 【WkD 迁移说明】
// 本文件由 ShadowStrike PhantomSensor SectionTracker.c 全量迁移。
...
//   * TimerManager (TmCreatePeriodic / TmCancel)
//     -> PeriodicTimer (CoCreatePeriodicTimer / WkdTimerStop / WkdTimerDestroy)
//   * HashUtils (ShadowStrikeHashContextInit/Update/Finalize/Cleanup)
//     -> BCrypt (BCryptOpenAlgorithmProvider / BCryptCreateHash /
//                BCryptHashData / BCryptFinishHash / BCryptDestroyHash)
//   * TeLogMemoryEvent -> 事件上报占位（DbgPrint），待接 ETW/ALPC 通知通道
// - 已删除 SS 专属 include：MemoryUtils.h、HashUtils.h、TelemetryEvents.h、
//   TimerManager.h、DriverEntry.h。
// - 定时器回调签名变更：SS (ULONG TimerId, PVOID Context) -> WkD (PVOID Context)
// - SEC_TRACKER_INTERNAL 新增 EX_RUNDOWN_REF（定时器生命周期管理所需）。
//
...
typedef struct _SEC_TRACKER_INTERNAL {
    ...
    //
    // 【WkD 迁移】SS 原用 ULONG CleanupTimerId + TimerManager;
    // WkD 替换为 WKD_PERIODIC_TIMER + EX_RUNDOWN_REF。
    //
    WKD_PERIODIC_TIMER CleanupTimer;
    ...
    )
{
    ...
    *Tracker = NULL;
    ...
    //
    // 【WkD 迁移】ExAllocatePool2 替换 ShadowStrikeAllocatePoolWithTag
    //
    internal = (PSEC_TRACKER_INTERNAL)ExAllocatePool2(
        NonPagedPoolNx,
    ...
    //
    // 【WkD 迁移】初始化 Rundown Ref（定时器生命周期管理）
    //
    ...
    internal->Public.SectionHash.Buckets = (PLIST_ENTRY)ExAllocatePool2(
        NonPagedPoolNx,
    ...
    //
    // 【WkD 迁移】SS 使用 TimerManager (TmCreatePeriodic)；
    // WkD 替换为 PeriodicTimer (CoCreatePeriodicTimer + CoStartPeriodicTimer)。
    // 定时器回调运行于 PASSIVE_LEVEL（Thread 模式）。
    //
    {
        NTSTATUS tmStatus = CoCreatePeriodicTimer(
            &internal->CleanupTimer,
            SEC_CLEANUP_INTERVAL_MS,
            SecpCleanupTimerCallback,
            internal,
            TRUE,           // RunAtPassive = TRUE
            &internal->RundownRef
        );
        if (NT_SUCCESS(tmStatus)) {
            CoStartPeriodicTimer(&internal->CleanupTimer);
        }
    }
...
Cleanup:
    if (internal != NULL) {
        ...
        //
        // 【WkD 迁移】定时器创建失败时销毁（SS 原为 TmCancel）
        //
        WkdTimerDestroy(&internal->CleanupTimer);

        ExFreePoolWithTag(internal, SEC_POOL_TAG_CONTEXT);
    }
    ...
    )
{
    ...
    //
    // 【WkD 迁移】停止清理定时器（SS 原为 TmCancel(tmMgr, timerId, TRUE)）。
    // WkdTimerStop 第二参数 TRUE = 等待正在执行的回调完成。
    //
    WkdTimerStop(&internal->CleanupTimer, TRUE);
    ...
    //
    // 【WkD 迁移】销毁定时器（SS 原为不需额外销毁，TimerManager 管理）
    //
    WkdTimerDestroy(&internal->CleanupTimer);
    ...
    )
{
    ...
    if (FileObject != NULL) {
        ...
        if (NT_SUCCESS(status) && nameInfo != NULL) {
            ...
            entry->BackingFile.FileName.Buffer = (PWCH)ExAllocatePool2(
                NonPagedPoolNx,
                entry->BackingFile.FileName.MaximumLength,
                SEC_POOL_TAG_ENTRY
            );
    ...
    )
{
    ...
    *FileDeleted = entry->BackingFile.IsDeleted;
    ...
    //
    // 【WkD 迁移】SS 原调用 TeLogMemoryEvent 上报注入检测事件。
    // WkD 暂以 DbgPrint 占位，待接 ETW (EtwWriteWkdMessage) 或
    // ALPC (WkNotificationServiceSendMessageAsync) 通知通道。
    //
    if (*IsTransacted || *FileDeleted) {
        DbgPrintEx(
            DPFLTR_IHVDRIVER_ID,
            DPFLTR_WARNING_LEVEL,
            "[WkDefender][SectionTracker] Doppelganging detected: "
            "CreatorPid=%p, SectionObj=%p, SuspicionScore=%lu, "
            "IsTransacted=%u, FileDeleted=%u\n",
            entry->CreatorProcessId,
            SectionObject,
            (ULONG)entry->SuspicionScore,
            (ULONG)*IsTransacted,
            (ULONG)*FileDeleted
        );
    }
...
/**
 * 【WkD 迁移】定时器回调签名变更：
 * SS: VOID (ULONG TimerId, PVOID Context)
 * WkD: VOID (PVOID Context) — 匹配 WKD_TIMER_CALLBACK 类型
 *
 * PeriodicTimer 线程模式保证回调运行于 PASSIVE_LEVEL。
 * 回调内部通过 RundownRef 保护生命周期。
 */
static VOID
SecpCleanupTimerCallback(
    _Inout_opt_ PVOID Context
...
CleanupPE:
    ...
    ExFreePoolWithTag(headerBuffer, SEC_POOL_TAG_CONTEXT);
}

// ============================================================================
// PRIVATE: FILE HASH (HIGH-1 fix — 【WkD 迁移】替换为 BCrypt 流式哈希)
// ============================================================================

/**
 * 【WkD 迁移】SS 使用 SHADOWSTRIKE_HASH_CONTEXT (Init/Update/Finalize/Cleanup)；
 * WkD 替换为内核 BCrypt API (BCryptCreateHash / BCryptHashData /
 * BCryptFinishHash / BCryptDestroyHash)。
 *
 * 每次调用打开独立的算法句柄 + 哈希对象，避免全局状态竞争。
 * 文件以 64KB 块增量读取并流式哈希，适用于大文件。
 */
static VOID
SecpComputeFileHash(
    _In_ PSECTION_ENTRY Entry,
...
CleanupHash:
    ...
    if (readBuffer != NULL) {
        ExFreePoolWithTag(readBuffer, SEC_POOL_TAG_CONTEXT);
    }
}
