/*++
    ...
    句柄范式:
        - 引擎以堆指针句柄（PWKD_ACCESS_CONTROL_ENGINE）对外暴露，
...
#include "../Callbacks/ProcessNotification.h"   /* CbGetProcessNotifyCallback */
#include "../Callbacks/ThreadNotification.h"    /* CbGetThreadNotifyCallback */
#include "../Callbacks/ImageNotification.h"     /* CbGetImageNotifyCallback */
...
SppStartSelfProtectionEngine(
    _In_ const PWKD_ACCESS_CONTROL_ENGINE Engine
    );
...
// 由 AcInitializeRegistryProtection 注入，内部查 AntiUnload：受保护进程自改自键放行。
//
static
BOOLEAN
...
//
struct _WKD_ACCESS_CONTROL_ENGINE {
...
//
static PWKD_ACCESS_CONTROL_ENGINE g_spEngineSession = NULL;
...
SpCreateSelfProtectionEngine(
    _Out_ PWKD_ACCESS_CONTROL_ENGINE* Engine
    )
{
    PWKD_ACCESS_CONTROL_ENGINE engine = NULL;
    ...
    *Engine = NULL;
    ...
    engine = (PWKD_ACCESS_CONTROL_ENGINE)ExAllocatePoolZero(
        NonPagedPoolNx,
        sizeof(WKD_ACCESS_CONTROL_ENGINE),
    ...
    )
{
    ...
    *Engine = NULL;
    ...
    //
    status = AcInitializeRegistryProtection(
...
/*++
    ...
    CbShutdownRegistryNotify），确保不再有新的 SpEngineShouldBlockRegistryAccess
    进入。NULL 安全。
    运行环境: PASSIVE_LEVEL
--*/
_Use_decl_annotations_
...
SpShutdownSelfProtectionEngine(
    _Inout_ PWKD_ACCESS_CONTROL_ENGINE Engine
    )
{
    PAGED_CODE();
...
{
    PWKD_ACCESS_CONTROL_ENGINE engine = g_spEngineSession;
    NTSTATUS status = STATUS_SUCCESS;
...
/*++
    ...
    由 AcInitializeRegistryProtection 注入，作为 WKD_RG_EXEMPT_CALLBACK。
...
/*++
    AcAuditRegistryAccess - 注册表操作分析总入口（CM 回调消费者入口）

    由 Callbacks/RegistryNotification.c 的 CM 回调调用（SetValue/DeleteValue 等
    携带值信息的操作）。内部获取引擎 rundown 后转调 RG
    （RgpAnalyzeRegistryAccess）完成操作分类、持久化/勒索/防御规避语义
    检测、按进程行为关联与统计累计。
    返回 TRUE 表示命中 BlockHighRiskOperations 配置对应的阻断点
    （勒索语义命中且该配置已开启），调用方可并入阻断决策。
    若引擎关闭中（rundown 获取失败）或 RG 未就绪，返回 FALSE。
    运行环境: PASSIVE_LEVEL
--*/
_Use_decl_annotations_
BOOLEAN
AcAuditRegistryAccess(
    _In_ PCUNICODE_STRING KeyPath,
    _In_ REG_NOTIFY_CLASS Operation,
    _In_ HANDLE RequestorProcessId,
    ...
    )
{
    ...
    if (!RequestorProcessId ||
        !CoCheckUnicodeStringValidity(KeyPath) ||
        (ValueName && !CoCheckUnicodeStringValidity(ValueName)) ||
        DataSize == 0) {
        return FALSE;
    }
    ...
    // 2026-09-08 RG 子模块：注册表操作分析转调（通知式：分类/持久化/勒索/
    // 防御规避语义 + 进程行为关联 + 统计）。
    if (engine->RegistryProtection != NULL) {
        shouldBlock = RgpAnalyzeRegistryAccess(
            engine->RegistryProtection,
            KeyPath,
            Operation,
            RequestorProcessId,
...
SpEngineProtectRegistryKey(
    PWKD_ACCESS_CONTROL_ENGINE Engine,
    PCWSTR KeyPath
    )
{
    PAGED_CODE();
