...
typedef struct _WKD_ACCESS_CONTROL_ENGINE WKD_ACCESS_CONTROL_ENGINE,
    *PWKD_ACCESS_CONTROL_ENGINE;
...
// 调用前提：外部必须已注销 OB 回调和 CM 回调（CbShutdownRegistryNotify），
...
SpShutdownSelfProtectionEngine(
    _Inout_ _Pre_valid_ _Post_invalid_ PWKD_ACCESS_CONTROL_ENGINE Engine
    );
...
SpEngineProtectRegistryKey(
    _In_ PWKD_ACCESS_CONTROL_ENGINE Engine,
    _In_ PCWSTR KeyPath
    );
