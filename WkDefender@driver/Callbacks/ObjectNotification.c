...
#include "../AccessControl/SelfProtectionEngine.h"   /* 自防护引擎：防卸载句柄访问剥离（经引擎暴露接口调用） */
...
/* ============================================================================
 * 掩码辅助（PpAccessAllowsInjection/Termination/MatchesCredentialDump/
 *           IsSafeReadOnly）
 * ============================================================================ */
...
//
// 后缀不敏感比较（ANSI 版，PppMatchImageNameSuffix 精神）
//
static BOOLEAN
PappMatchImageNameSuffixA(
    _In_ PCCHAR ImagePath,
...
// （PpDetermineVerdict；Block 为预留，当前不产出）
//
static WKD_PAP_VERDICT
PappDetermineVerdict(
    _In_ ULONG SuspicionScore,
    ...
    )
{
    ...
    //
    // 默认配置（PppInitializeDefaultConfig）
    //
...
// ObjectCallback.c 的 HtRecordDuplication 触发点（OB 回调 DUPLICATE 分支）。
// 死代码：本模块 CbInitializeObjectNotify 未激活；跨进程复制关联由 Agent
// 因果图消费（PrAddRelationship(PrRelation_HandleDuplication)），
// 内核不维护复制记录列表。
//
_Use_decl_annotations_
static
    ...
    )
{
    ...
    /* 异步上送（复制事件仅记录，不做同步阻塞，HtRecordDuplication 纯记录语义） */
    NtfSendMessageAsync(message);
}
...
//
// 攻击模式判定（TpDetectAttackPattern 判定顺序：
...
//
// 可疑标志分析与打分（TpAnalyzeOperation + TppCalculateSuspicionScore）。
    ...
    )
{
    ...
    //
    // 攻击类型加成（TppCalculateSuspicionScore switch）
    //
...
//
// 等级剥离矩阵（TpCalculateAllowedAccess，输出 DeniedMask 形式）。
...
//
// 剥离日志（TppLogOperation 精简版；原内联于引擎入口，上移后提取，
// 签名直传标量，不依赖任何中转结构）。
//
static VOID
...
--*/
{
    ...
    if (OperationInformation->Operation == OB_OPERATION_HANDLE_CREATE) {
        ...
        /* 句柄复制事件上送（ObjectCallback.c 两处 HtRecordDuplication，
         * HandleTracker 迁移 2026-08）。死代码：本模块 CbInitializeObjectNotify
         * 被 WkdEntry 注释未激活；跨进程复制关联由 Agent 因果图消费
         * （PrAddRelationship(PrRelation_HandleDuplication)）。 */
        //CbpNotifyIoaHandleDuplicate(
        //    sourceProcessId,
