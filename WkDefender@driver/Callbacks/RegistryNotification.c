/*++
    Callbacks/RegistryNotification.c - 注册表回调节点实现
    ...
    Purpose:
        ...
        5. 对 SetValue/DeleteValue 提取值名/数据/类型，调用分析入口
           AcAuditRegistryAccess（分类/持久化/勒索语义/进程行为
           关联/统计）。其返回 TRUE（命中 BlockHighRiskOperations 阻断点）
           同样并入阻断决策。
    ...
    Synchronization:
        - g_RegistryInitialized / g_RegistryLock (FAST_MUTEX) 串行化初始
          化与清理；CbRegistryNotificationCallback 由 CM 在 PASSIVE_LEVEL 调用，
          卸载钩子后不再有并发进入。

    Copyright (c) WkDefender Team
--*/

#include "RegistryNotification.h"
#include "../Common/Utils.h"     /* WkdDriverObject 全局（驱动对象句柄） */
#include "../AccessControl/SelfProtectionEngine.h"  /* 自防护引擎公共入口（BLOCK 判定消费） */
#include <ntstrsafe.h>

#ifdef ALLOC_PRAGMA
#pragma alloc_text(PAGE, CbInitializeRegistryNotify)
#endif
...
static
NTSTATUS
CbpGetRegistryKeyObjectPath(
    _In_ const PVOID KeyObject,
    ...
    )
{
    ...
    *KeyPath = NULL;
    ...
    nameInfo = (POBJECT_NAME_INFORMATION)ExAllocatePoolZero(
        PagedPool, returnLength, WKD_REG_POOL_TAG);
    if (!nameInfo) return STATUS_NO_MEMORY;

    /* 第二次实际填充 */
    status = ObQueryNameString(KeyObject, nameInfo, returnLength, &returnLength);
    if (!NT_SUCCESS(status)) goto Cleanup;

    status = CoCopyUnicodeString(&keyPath, &nameInfo->Name);
    if (!NT_SUCCESS(status)) goto Cleanup;
...
NTSTATUS
CbRegistryNotificationCallback(
    ...
    )
{
    ...
    /* 值信息（仅 SetValue/DeleteValue 携带；其余操作保持 NULL/0） */
    PCUNICODE_STRING valueName = NULL;
    PVOID valueData = NULL;
    ULONG valueDataSize = 0;
    ULONG valueDataType = 0;
    ...
    switch (notifyClass) {
        ...
        case RegNtPreSetValueKey:
            /* SetValue：同时提取值名/数据/类型供分析入口（回调返回前有效） */
            setValueInfo = (PREG_SET_VALUE_KEY_INFORMATION)Argument2;
            keyObject = setValueInfo->Object;
            valueName = setValueInfo->ValueName;
            valueData = setValueInfo->Data;
            valueDataSize = setValueInfo->DataSize;
            valueDataType = setValueInfo->Type;
            break;

        case RegNtPreDeleteValueKey:
            /* DeleteValue：提取值名供分析入口 */
            deleteValueInfo = (PREG_DELETE_VALUE_KEY_INFORMATION)Argument2;
            keyObject = deleteValueInfo->Object;
            valueName = deleteValueInfo->ValueName;
            break;
    ...
    if (keyObject == NULL) return STATUS_SUCCESS;

    /* 解析父键路径 */
    status = CbpGetRegistryKeyObjectPath(keyObject, &keyPath);
    if (!NT_SUCCESS(status)) return STATUS_SUCCESS;

    /* CreateKeyEx：拼接完整路径 = RootObject + "\\" + CompleteName */
    if (notifyClass == RegNtPreCreateKeyEx) {
        createInfo = (PREG_CREATE_KEY_INFORMATION)Argument2;
        if (CoCheckUnicodeStringValidity(createInfo->CompleteName)) {
...
BlockAdvise:
    /* 1. 交给自防护引擎 BLOCK 判定（引擎内部复用 rundown，外部异步入口语义一致） */
    shouldBlock = SpEngineShouldBlockRegistryAccess(&keyPath, notifyClass, pid);

    /* 2. 通知式分析：SetValue/DeleteValue 已提取值名/数据/类型，其余操作
     *    传 NULL/0。分析命中 BlockHighRiskOperations 阻断点（勒索语义）
     *    时并入阻断决策。 */
    if (AcAuditRegistryAccess(
            &keyPath,
            notifyClass,
        pid,
            valueName,
    ...
    )
{
    ...
    if (!NT_SUCCESS(status)) {
        ...
        DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL,
            "[WkDefender] CbInitializeRegistryNotify: CmRegisterCallbackEx failed: 0x%08X\n",
            status);
        return status;
    }
    ...
    )
{
    ...
    if (!NT_SUCCESS(status)) {
        DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_WARNING_LEVEL,
            "[WkDefender] CbShutdownRegistryNotify: CmUnRegisterCallback failed: 0x%08X (cookie=0x%I64X)\n",
