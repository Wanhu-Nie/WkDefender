...
#include "../Include/FileSystem.h"   /* 文件系统子系统对外公共头：FsBackupCommitProcess（2026-09-13 重构） */
#include "../Process/ProcessAccessProtection.h"  /* Pap 进程退出清理钩子 */
#include "../AnalysisEngine/IocProcess.h"  /* T1115 剪贴板追踪表清理（IocpClipboardRemoveProcess，2026-10 接线） */
    ...
    )
{
    ...
    /* 镜像加载回调已迁至独立模块 Callbacks/ImageNotification.c（DEC-05），
...
// [死代码] TeLogRemoteThread/TeLogThreadCreate（ThreadNotify.c L1202-1218）。
...
// [死代码] TeLogThreadExit 语义；不接线原因同 CbEtwEmitThreadCreate。
//
_Use_decl_annotations_
VOID
CbEtwEmitThreadExit(
    ...
    )
{
     ...
     * 释放自动消失，无需独立追踪表清理（WSLMonitor 2026-08）。 */
    ...
    /* [ProcessNotify→ClipboardMonitor，2026-10 激活] T1115 剪贴板追踪表清理
     * （IocProcess.c §8）——随 IRP_MJ_WRITE 接线 + IocpClipboardTrackProcess
     * 入表启用；防追踪表填满后失聪 + PID 复用误判。 */
    IocpClipboardRemoveProcess(ProcessId);
     ...
     * （FsBackupCommitProcess 定义于 FileSystem\FileSystem.c 编排器封装，
     *   2026-09-13 重构后经 FsBackupCommitProcess 暴露；暂不参与编译） */
#if 0 /* [FileSystem 排除-暂存代码] */
    /* FBE 进程退出提交：丢弃备份（PnpHandleProcessTermination
     * →FbeCommitProcess，SS ProcessNotify.c:4034）。PASSIVE_LEVEL，
     * 早于 PmHashMapRemove 释放上下文（防 PID 复用误回滚）。 */
    FsBackupCommitProcess(ProcessId);
...
--*/
{
    /* 防御性检查（ShadowStrikeProcessNotifyCallback L1128-1130） */
    if (Process == NULL) {
        return;
    }
    ...
    if (!isCreation) {
        ...
        // ShadowStrikeIsServiceConnected（ProcessNotify.c:184-191,1159-1167）。
...
//
// [死代码] 大小写不敏感子串搜索（PnpSafeWcsStrI L4504-4571；
...
//
// [死代码] 已知系统进程检查（PnpIsKnownSystemProcess L4233-4269）
...
//
// [死代码] 信任否决阻断判定（PnpIsTrustedProcess L4272-4343）
...
//
// [死代码] 周期特权/令牌变化巡检（PnpPeriodicSecurityChecks L4637-4828）
    ...
    )
{
    ...
    /* 新启用的危险特权 = 运行时提权迹象（PmCheckForEscalation 语义） */
    for (ULONG i = 0; i < 4; i++) {
...
//
// [死代码] 创建时进程保护分类（主回调 L1312-1321 PpClassifyProcess/
    ...
    )
{
    ...
    /* PpClassifyProcess：按镜像名分类关键进程 + 注册保护。
